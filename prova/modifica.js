...
function sdpCompatto(sdp){
  ...
  while ((m = re.exec(sdp))){
    ...
    if (uuid) cand.push({ ord: 0, s: 'm' + b64urlDaByte(uuid.slice(1).join('').match(/../g).map(h => parseInt(h, 16))) + '~' + porta });
    /* i punti dell'IP diventano trattini: il punto e' il separatore dei campi
       (trovato dal test, non dal ragionamento) */
    else if (/^\d+\.\d+\.\d+\.\d+$/.test(addr)) cand.push({ ord: 1, s: 'i' + addr.replace(/\./g, '-') + '~' + porta });
...
async function renderBurners(){
  ...
  for (const b of burners()){
    ...
    qr.addEventListener('click', () => salvaQrIndirizzo(addr, b.name || t('burn.untitled','Senza nome')));
...
$('btnAddrSaveQr').addEventListener('click', () => salvaQrIndirizzo());
