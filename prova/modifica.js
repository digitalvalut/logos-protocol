...
"home.inviteLink":"دعوة محدودة المدة، دون إعطاء أي عنوان","welcome.title":"مرحبًا. ما اسمك؟","welcome.go":"ابدأ","welcome.why":"اسمك لا يراه إلا من تتصل بهم. بعدها يكون لديك عنوانك فورًا لترسله لمن تشاء.","home.tileBurner":"عنوان مؤقت","home.addrLine":"أعطاك أحد عنوانًا مكتوبًا؟ اكتبه هنا","home.otherWays":"طرق أخرى","home.cardShare":"أرسل عنواني","home.bigBook":"اتصل بشخص تعرفه","home.joinLink":"أرسل لك أحدهم رابطًا لا يُفتح؟ الصقه هنا","set.lang":"اللغة","set.grpYou":"من أنت","set.grpAdv":"المزيد","set.textsize":"حجم النص","conn.direct":"اتصال مباشر بين الهاتفين","conn.directShort":"متصل مباشرة","conn.relay":"اتصال آمن عبر جسر مشفَّر","conn.relayShort":"متصل (جسر)","conn.down":"انقطع الاتصال","conn.downShort":"منقطع","conn.working":"جارٍ الاتصال","conn.wobbly":"تذبذب الاتصال — أحاول استعادته","conn.wobblyShort":"جارٍ الاستعادة","chat.linkLost":"انقطع الاتصال. لم يُفقد شيء.","chat.backHome":"العودة إلى الصفحة الرئيسية","chat.stillWobbly":"لم يعد الاتصال بعد. يمكنك الانتظار أكثر، أو العودة إلى الصفحة الرئيسية والمحاولة من جديد.","call.lostForGood":"انقطعت المكالمة ولم يتسنَّ استعادتها.","chat.lostForGood":"انقطع الاتصال ولم يتسنَّ استعادته.",
...
function showScreen(id){
  ['screenHome','screenSettings','screenContacts','screenStart','screenJoin','screenChat'].forEach(s => $(s).classList.toggle('hide', s !== id));
...
/* ---------------- il lasciapassare che scade sotto una conversazione ----------------
   ⚠️ TROVATO IL 23 SET 2026, cercando perche' una videochiamata cadeva dopo
   cinque minuti, o dodici, senza un motivo visibile.

   Una RTCPeerConnection si porta dietro per tutta la vita la configurazione
   con cui e' nata (`newPeerConnection`), credenziali del ponte comprese. Fin
   qui e' innocuo: se i due telefoni si parlano diretti, del ponte non importa
   niente e nessuno si accorge di nulla. Ma quando la rete li costringe al
   ponte — due operatori mobili diversi, un ufficio dietro un NAT severo — la
   conversazione vive esattamente quanto quelle credenziali, e non un secondo
   di piu'. Quanto duri lo dice `TURN_TTL_SECONDS` nel relay.

   Il guaio peggiore pero' non era la caduta: era che la RIPRESA non poteva
   riuscire. `startRepair` rifa' la trattativa sulla stessa connessione, con la
   stessa configurazione, e quindi ripresentava al ponte un lasciapassare gia'
   morto: tre giri, un minuto l'uno, tutti persi in partenza. Da fuori si vede
   una chiamata che cade e non torna piu', con lo schermo che dice «sto
   riprendendo» per tre minuti senza riprendere niente — la cosa che fa
   sembrare rotta un'app che invece sa benissimo rimettersi in piedi.

   `setConfiguration` sostituisce le credenziali sulla connessione viva. Non
   resuscita il canale gia' aperto sul ponte, quello e' perduto; ma il restart
   ICE che viene subito dopo ne apre uno nuovo con un lasciapassare valido, ed
   e' tutta la differenza fra una chiamata che riprende in qualche secondo e
   una che e' finita.

   Perche' NON e' anche un rinnovo periodico, che sarebbe la cura completa:
   rifare la trattativa su una chiamata che sta funzionando e' il modo classico
   di rompere una chiamata che sta funzionando — questo file ci e' gia' cascato
   il 12 set 2026 e la cicatrice e' scritta sopra `onConnectionStateChange`.
   Qui si interviene solo dove la connessione e' gia' caduta, dove non c'e'
   niente da rompere. Il margine per non arrivarci affatto sta nel relay. */
async function refreshIceConfig(conn){
...
/* ---------------- rinnovare il lasciapassare DENTRO una chiamata ----------------
   ⚠️ SCRITTO, ROTTO E RISCRITTO IL 23-24 SET 2026. Leggere prima di toccarlo:
   questa e' la terza stesura, e le prime due hanno rotto l'app in modi che
   una suite tutta verde non poteva vedere.

   Il problema. Una RTCPeerConnection tiene per tutta la vita la configurazione
   con cui e' nata, credenziali del ponte comprese, e quelle durano
   `TURN_TTL_SECONDS` = 600 secondi. Chi e' costretto al ponte ha davanti quei
   minuti e non uno di piu'. MISURATO su due dispositivi veri il 23 set:
   immagine congelata a 10:35, caduta dichiarata a 11:42. Non e' una teoria.

   Sostituire le credenziali non basta: il canale gia' aperto sul ponte
   continua con le vecchie e muore con loro. Per averne uno nuovo serve una
   rinegoziazione — e una rinegoziazione su una connessione viva non e' gratis.
   Le due lezioni pagate cara:

   1. UNA RINEGOZIAZIONE AZZERA I PARAMETRI DEI SENDER. Il video perde il tetto
      di CALL_VIDEO_MAX_BPS, la salita mobile satura, l'immagine si pianta —
      mentre il cronometro conta e la telecamera gira, cioe' con la connessione
      viva. Percio' `ritocca()` subito dopo, da tutti e due i lati. E per la
      stessa ragione `ensureOpusFec` sul proprio SDP: una rinegoziazione
      porterebbe via la correzione d'errore della voce proprio quando la rete
      perde pacchetti.

   2. MENTRE LA RINEGOZIAZIONE E' IN CORSO la connessione sta in
      'have-local-offer' per qualche secondo, e in quella finestra UNA CHIAMATA
      NUOVA NON PARTE: uno squilla, l'altro non riceve niente. La prima stesura
      rinnovava sempre, anche da fermi, e ha rotto esattamente questo.
      Da qui la regola che tiene in piedi tutto il resto: si rinnova SOLO
      mentre una chiamata e' attiva — dentro una chiamata non c'e' nessuna
      chiamata nuova da bloccare. L'aggancio e' `startCallTimer`/`stopCallTimer`,
      cosi' il rinnovo vive esattamente quanto la chiamata: un posto solo, da
...
async function renewIceNow(conn){
  /* credenziali VERAMENTE nuove: fetchIceServers() restituirebbe quelle in
     cache, che possono essere vecchie quanto quelle da sostituire */
  let fresh = null;
...
/* L'altro lato. Rinfresca anche lui: i due capi del ponte sono due
   allocazioni distinte, e una sola rinnovata non tiene su niente. */
...
/* ⚠️ IL PUNTO DA CUI DEVE PASSARE CHIUNQUE RINEGOZI UNA CHIAMATA VIVA.
   Una rinegoziazione azzera i parametri dei sender: senza rimetterli, il video
   riparte senza tetto, la salita satura e l'immagine si congela a meta'
   chiamata. E' successo davvero, il 23 set 2026. Esiste come funzione, e non
   come riga copiata in tre punti, perche' la quarta strada che verra' aggiunta
   un giorno se ne dimenticherebbe. */
async function ritocca(conn){
  if (pc !== conn || conn.connectionState === 'closed') return;
  try{ await tuneSendersForMobile(conn); }catch(e){}
}
/* Come refreshIceConfig, ma saltando la cache: qui le credenziali vecchie sono
   esattamente il problema, non una scorciatoia accettabile. */
  ...
  if (conn.connectionState === 'disconnected'){
    ...
    if (conn.connectionState === 'failed'){
      /* l'altro lato e' una versione che non sa riprendere: non c'e' niente
         da tentare, e restare zitti qui e' esattamente come il microfono
         restava acceso su una chiamata finita */
      if (!repairBase) giveUpOnConnection(conn);
      else startRepair(conn);
    }
  }
}
...
async function startRepair(conn){
  ...
  try{
    /* Prima di ogni altra cosa, e da tutti e due i lati perche' tutti e due
       raccoglieranno indirizzi nuovi: le credenziali del ponte possono essere
  ...
  }catch(e){ /* un tentativo fallito lascia tutto com'era: ci pensa il giro dopo */ }
  finally{
    conn.__repairing = false;
    /* ⚠️ LA RIGA CHE MANCAVA, misurata il 24 set 2026 dopo che una
       videochiamata vera e' caduta e non e' piu' tornata.

       Il tetto qui sopra dice tre giri. Ne veniva fatto UNO. `startRepair`
       partiva solo da `onConnectionStateChange`, cioe' da un CAMBIO di stato
       — ma una connessione gia' 'failed' non cambia piu' stato: per
       specifica ci resta. Quindi il primo giro andava a vuoto, il secondo e
       il terzo non esistevano, e la conversazione restava giu' per sempre
       con tre tentativi ancora in tasca e nessuno che li spendesse.
       Il commento sopra diceva «il prossimo stato lo ritenta»: il prossimo
       stato non arrivava mai. Un tetto che non si raggiunge mai non e' un
       tetto, e' un pavimento.

       Misurato: `__repairRounds` fermo a 1, una sola busta scritta.
       Adesso il giro dopo se lo chiede da solo. */
    scheduleNextRepair(conn);
  }
}

/* Il giro successivo, quando quello appena finito non ha ricongiunto niente.
   Non e' una pompa che gira a vuoto: si ferma da sola appena la connessione
   torna su, quando viene chiusa, o quando i tentativi sono finiti — e in
...
function scheduleNextRepair(conn){
  if (pc !== conn || !repairBase) return;
  if (conn.connectionState === 'connected' || conn.connectionState === 'closed') return;
  if (conn.__nextRepairTimer) return;                    /* uno solo in coda */
  const finiti = repairBase.offerer
    ? (conn.__repairRounds || 0) >= REPAIR_MAX_ROUNDS
    : (conn.__repairListens || 0) >= REPAIR_MAX_LISTENS;
  if (finiti){ giveUpOnConnection(conn); return; }
  conn.__nextRepairTimer = setTimeout(() => {
    conn.__nextRepairTimer = null;
    startRepair(conn);
  }, REPAIR_RETRY_GAP_MS);
}
function stopRepairRetries(conn){
  if (conn && conn.__nextRepairTimer){ clearTimeout(conn.__nextRepairTimer); conn.__nextRepairTimer = null; }
}

/* ---------------- quando non c'e' davvero piu' niente da riprendere ----------------
   ⚠️ NATO IL 24 SET 2026, dalla stessa prova. Finiti i tentativi, il codice
   semplicemente smetteva, e non lo diceva a nessuno. Su una chiamata: il
   cronometro continuava a contare i minuti di una conversazione finita, il
   microfono e la fotocamera restavano accesi, e il telefono restava in
   modalita' chiamata — altoparlante commutato, schermo che non si spegne,
   batteria che si consuma — finche' la persona non trovava da sola «Torna
   alla home». Tre cose che dicono «stai parlando con qualcuno» a chi non sta
   parlando con nessuno.

   `endCall` sapeva gia' rimettere tutto a posto da qualunque strada: mancava
   solo che qualcuno la chiamasse anche da questa. `tellPeer` e' false di
   proposito — il canale e' morto, non c'e' nessuno da avvisare.

   Si chiude la CHIAMATA, non la conversazione: quello che era stato scritto
   resta sullo schermo finche' e' la persona a decidere. */
function giveUpOnConnection(conn){
  if (pc !== conn) return;
  if (conn.__gaveUp) return;              /* le strade che arrivano qui possono arrivarci insieme */
  conn.__gaveUp = true;
  const eraInChiamata = (typeof callState === 'string' && callState !== 'idle');
  endCall(false);
  if (!$('screenChat').classList.contains('hide')){
    sysLineWithAction(
      eraInChiamata
        ? t('call.lostForGood','La chiamata è caduta e non è stato possibile riprenderla.')
        : t('chat.lostForGood','Il collegamento è caduto e non è stato possibile riprenderlo.'),
      t('chat.backHome','Torna alla home'),
      endSession);
  }
}
...
async function renderBurners(){
  ...
  for (const b of burners()){
    ...
    /* Anche l'usa e getta si stampa — un annuncio, una bacheca, un biglietto
       per uno sconosciuto: e' esattamente il posto di un indirizzo che poi si
       butta. Quando l'annuncio finisce lo si cancella, e chi aveva fotografato
       il foglio non trova piu' nessuno. */
    const qr = document.createElement('button');
    qr.textContent = '🖼';
    qr.title = t('burn.saveQr','Salva il QR di questo indirizzo');
    qr.setAttribute('aria-label', qr.title);
    /* Sotto il QR stampato va il NOME che gli hai dato: una seconda casella
       «scritta da stampare» accanto a quella del nome erano due campi che
       chiedevano la stessa cosa, e chi legge non poteva sapere quale conta.
       Uno solo, e serve per tutte e due le cose. */
...
async function salvaQrIndirizzo(addr, etichetta){
  ...
  /* Sotto l'indirizzo va quello che serve a chi guarda il foglio, non a noi:
     su un annuncio o un biglietto «DV-…» da solo non dice niente, e il nome
     interno di un usa e getta nemmeno. Quindi lo sceglie chi stampa. */
...
function endSession(){
  ...
  $('destructCountdown').classList.add('hide'); $('btnDisarmDestruct').classList.add('hide');
  stopQuickPump();
  stopRepairRetries(pc);   /* i giri in coda appartenevano a questa conversazione */
  endCall(false);
  if (dc) try{ dc.close(); }catch(e){}
  if (pc) try{ pc.close(); }catch(e){}
