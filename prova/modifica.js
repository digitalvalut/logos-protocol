...
function sdpCompatto(sdp){
  ...
  while ((m = re.exec(sdp))){
    ...
    else if (/^\d+\.\d+\.\d+\.\d+$/.test(addr)) cand.push({ ord: 1, s: 'i' + addr + '~' + porta });
    else if (/^[0-9a-f:]+$/i.test(addr)) cand.push({ ord: 2, s: 'j' + addr + '~' + porta });
  }
...
function sdpEspanso(compatto, ruolo){
  ...
  for (const c of indirizzi.split(',')){
    ...
    } else if (m[1] === 'i'){ if (!/^\d{1,3}(\.\d{1,3}){3}$/.test(m[2])) return null; addr = m[2]; }
    else { if (!/^[0-9a-fA-F:]{2,45}$/.test(m[2])) return null; addr = m[2]; }
...
async function salvaQrIndirizzo(){
  ...
  const canvas = immagineQr(m, scritto, 'DigitalValut Logos');
