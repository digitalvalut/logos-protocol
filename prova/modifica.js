...
"easy.title":"简单模式","easy.sub":"只有两个大按钮，周围什么都没有。给不想操心的人——也给替别人设置手机的人。","easy.voiceTitle":"读出来","secure.title":"屏幕保护","secure.sub":"禁止截屏，最近任务中显示为黑屏。默认关闭：开启后，邀请二维码也无法截屏。","easy.voiceSub":"应用会用你的语言把该做的事读出来。适合看屏幕吃力的人。","easy.voiceOn":"好的。从现在起我会把该做的事读给你听。","easy.sayHome":"点第一个按钮开始聊天。如果别人给你发了邀请，就点第二个。","easy.sayStart":"这是你的号码。按橙色按钮把它发给你想发的人。","easy.sayJoin":"输入别人给你的六位数字。","easy.sayChat":"已经连上了。现在可以说话了。","easyhint.text":"还有一种只有两个大按钮的简单模式，适合不想多想的人。","easyhint.btn":"开启",
...
"easy.title":"आसान मोड","easy.sub":"बस दो बड़े बटन और आसपास कुछ नहीं। उनके लिए जो कुछ सोचना नहीं चाहते — या जो किसी और का फ़ोन तैयार कर रहे हैं।","easy.voiceTitle":"बोलकर बताओ","secure.title":"सुरक्षित स्क्रीन","secure.sub":"कोई स्क्रीनशॉट नहीं, और खुले ऐप्स में काला पूर्वावलोकन। डिफ़ॉल्ट रूप से बंद: चालू करने पर आमंत्रण के QR का स्क्रीनशॉट भी नहीं ले सकते।","easy.voiceSub":"ऐप आपकी भाषा में बोलकर बताता है कि क्या करना है। उनके लिए जिन्हें स्क्रीन पढ़ना मुश्किल लगता है।","easy.voiceOn":"ठीक है। अब से मैं बोलकर बताऊंगा कि क्या करना है।","easy.sayHome":"बातचीत शुरू करने के लिए पहला बटन दबाएं। अगर किसी ने आपको निमंत्रण भेजा है तो दूसरा दबाएं।","easy.sayStart":"यह आपका कोड है। जिसे चाहें भेजने के लिए नारंगी बटन दबाएं।","easy.sayJoin":"आपको दिए गए छह अंक लिखें।","easy.sayChat":"जुड़ गए। अब आप बात कर सकते हैं।","easyhint.text":"एक ऐसा तरीका भी है जिसमें सिर्फ़ दो बड़े बटन होते हैं, उनके लिए जो इसके बारे में सोचना नहीं चाहते।","easyhint.btn":"चालू करें",
...
function easyPref(){ try{ return MEM.getItem('dvlogos-easy') === '1'; }catch(e){ return false; } }
...
$('easyRow').addEventListener('click', () => {
  ...
  try{ MEM.setItem('dvlogos-easy', on ? '1' : '0'); }catch(e){}
  applyEasy(on);
});
...
for (const id of ['easyRow','voiceRow','secureRow']){
...
function maybeShowEasyHint(){
  if (easyPref()) return;
  let dismissed = false; try{ dismissed = MEM.getItem('dvlogos-easyhint-dismissed') === '1'; }catch(e){}
  if (dismissed) return;
...
$('easyHintBtn').addEventListener('click', () => {
  try{ MEM.setItem('dvlogos-easy', '1'); }catch(e){}
  applyEasy(true);
  $('easyHintBar').classList.add('hide');
  try{ MEM.setItem('dvlogos-easyhint-dismissed', '1'); }catch(e){}
});
...
/* ⚠️ NON E' UNA RUBRICA — vedi il commento accanto a #addrPeople nella pagina.
   Una riga sola di nomi sotto il campo dell'indirizzo: e' il campo che si
   ricorda chi chiami di solito, come fa il telefono con l'ultimo numero.
   Niente avatar, niente «3 giorni fa», niente × che cancella: quella roba e'
   rimasta per intero nella rotellina, che e' il posto dove si mette ordine.
   Ci finiscono TUTTI i contatti salvati, non solo quelli con un indirizzo:
   chi ha solo l'impronta si raggiunge lo stesso, per un'altra strada e solo
   se ha l'app aperta in quel momento — toglierli avrebbe tolto l'unico modo
   di richiamare qualcuno conosciuto prima che gli indirizzi esistessero. */
function renderAddrPeople(list){
  const box = $('addrPeople');
  if (!box) return;
  box.classList.toggle('hide', list.length === 0);
  if (!list.length){ box.innerHTML = ''; return; }
  const collide = contactCollisions(list);
  /* Otto e non quaranta: sotto un campo di testo questa e' una scorciatoia,
     e una scorciatoia lunga venti righe torna a essere l'elenco che abbiamo
     appena tolto dalla prima pagina. Chi cerca il nono va nella rotellina. */
  box.innerHTML = list.slice(0, 8).map(c => {
    const bad = collide(c) === 'bad';
    /* Il segno sta DENTRO il pulsante e prima del nome, non accanto: qui si
       tocca per chiamare, e un avviso che si legge dopo aver toccato non e'
       un avviso. Il titolo lo spiega a chi si ferma un secondo. */
    return `<button class="chip${bad ? ' warn' : ''}" data-reach="${esc(c.nick)}"`
         + (bad ? ` title="${esc(t('contacts.sameLookUnverified','mai verificato — nome quasi identico a un altro'))}"` : '')
         + `>${bad ? '<span class="wm" aria-hidden="true">!</span>' : ''}${esc(c.nick)}</button>`;
  }).join('');
}
...
/* I nomi sotto il campo dell'indirizzo, sulla prima pagina. Un tocco = provo
   ...
   che non c'e'. */
$('addrPeople').addEventListener('click', ev => {
  const chip = ev.target.closest('[data-reach]');
  if (!chip) return;
  reachContact(chip.getAttribute('data-reach'));
});
...
/* ============================== how the app is doing ==============================
   ...
   guessing. */
const APP_VERSION = 'logos-modifica-4.44';
