...
"prot.working":"Идёт работа… это займёт несколько секунд: именно эта медлительность делает перебор слов бессмысленным.","prot.lock":"Убрать","prot.title":"Убрать разговор с «{name}»","prot.what1":"Разговор исчезает с телефона: сообщения, имя контакта и проверка из трёх слов. Тот, кто возьмёт телефон, не найдёт ни разговора, ни контакта.","prot.what2":"Фотографии и файлы <b>не удаляются</b>: они попадают в сейф зашифрованными и возвращаются при открытии.","prot.what3":"Вы выбираете слово. <b>Забудете — не откроет никто</b>: ни вы, ни мы. Восстановления не существует.","prot.what4":"Тот, кто просматривает приложение, не найдёт ничего. Тот, у кого есть криминалистические инструменты, поймёт, что что-то убрано, но не что и не с кем.","prot.wordPh":"Слово, которое откроет обратно","prot.word2Ph":"Повторите","prot.go":"Убрать","prot.cancel":"Отмена","prot.needWord":"Напишите слово: без него сейфа нет.","prot.short":"Слишком коротко. Не меньше 8 символов — лучше целая фраза, которую помните только вы.","prot.mismatch":"Слова не совпадают.","prot.done":"«{name}» убран. Вернёт его именно это слово.","prot.failed":"Не получилось. Ничего не удалено: разговор на месте.","prot.tooBig":"Не получилось: {n} файлов слишком велики для сейфа (больше 64 МБ). Ничего не тронуто. Удалите их из разговора и попробуйте снова.","prot.openTitle":"Открыть убранный разговор","prot.openHelp":"Напишите слово. Если ничего не совпадёт, ничего не произойдёт: приложение не скажет, ошиблись вы или там ничего не было.","prot.openGo":"Открыть","prot.openNothing":"С этим словом ничего не открывается.","prot.openDone":"«{name}» вернулся в список вместе с фотографиями.","prot.openLost":"«{name}» вернулся, но {n} файлов не открылись.","prot.openFailed":"Не получилось. Сейф остался полным: попробуйте, когда на телефоне будет больше места.",
...
async function protectConversation(fp, nick, word){
  ...
  const vaultConvKey = 'dvlogos-vm-' + await slotId(sec.seed, 'media');
  ...
  /* Qualcosa non ci stava. Non si decide qui cosa farne: si torna indietro
     pulendo quello che si era gia cifrato, e si riferisce. Chi guarda lo schermo
     sceglie se cancellare quei file o lasciare la conversazione scoperta —
     nessuna delle due e una scelta che questa funzione puo prendere per lui. */
  if (media.tooBig.length){
    try{ await mediaDeleteByConv(vaultConvKey); }catch(e){}
    return { ok: false, tooBig: media.tooBig };
  }

  bundle.mediaKey = vaultConvKey;
  if (!await vaultPut(word, bundle, sec)){
    try{ await mediaDeleteByConv(vaultConvKey); }catch(e){}
  ...
  /* La rilettura non e pedanteria: e l'unica prova che quello che sta per
     ...
     lo stesso. */
  const check = await vaultGet(word, sec);
  if (!check || check.fp !== fp || check.history !== history || check.safety !== safety){
    try{ await mediaDeleteByConv(vaultConvKey); }catch(e){}
    return { ok: false, tooBig: [] };
  }
  ...
  /* Solo adesso: la copia cifrata esiste ed e stata riletta. Quello che sparisce
     qui e l'originale in chiaro, non l'unica copia. */
  try{ await mediaDeleteByConv(hKey); }catch(e){}
  return { ok: true, tooBig: [], media: media.count };
}
...
/* Riporta allo scoperto una conversazione messa al riparo: torna nella lista
   dei contatti come se non se ne fosse mai andata.

   L'ORDINE E' LO STESSO DI PRIMA, ROVESCIATO E CON LA STESSA REGOLA: si scrive
   tutto in chiaro, si RILEGGE, e solo dopo si svuota la cassetta. Al contrario
   — svuotare e poi scoprire che il telefono non aveva piu memoria — vorrebbe
   dire perdere la conversazione nel momento in cui si chiedeva di riaverla. */
async function restoreProtected(word){
  /* Una sola volta, e passata a mano a tutti quelli che ne hanno bisogno.
     Ricavarla dalla parola costa apposta molto — e' quello che rende inutile
     provare le parole a raffica — ma ricavarla TRE volte per una sola
     operazione non protegge nulla di piu': fa solo aspettare chi ha il
     diritto di entrare. Trovato usando l'app, non leggendola. */
  const sec = await vaultSecrets(word);
  const b = await openProtected(word, sec);
  if (!b) return { ok: false, wrong: true, persi: [] };
  ...
  if (b.mediaKey){
    ...
    if (!m.ok) return { ok: false, wrong: false, persi: m.persi };
    persi = m.persi;
  }
  ...
  }catch(e){ return { ok: false, wrong: false, persi }; }

  /* La stessa rilettura del cammino d'andata, per la stessa ragione: e la
     prova che quello che sta per essere svuotato esiste di nuovo altrove. */
  if (b.history != null && localStorage.getItem(hKey) !== b.history){
    return { ok: false, wrong: false, persi };
  }

  try{ await vaultDrop(word, sec); }catch(e){}
  if (b.mediaKey){ try{ await mediaDeleteByConv(b.mediaKey); }catch(e){} }
  return { ok: true, wrong: false, persi, nick: b.nick, fp: b.fp };
}
...
function openProtectBox(nick){
  ...
  $('protectWord').value = ''; $('protectWord2').value = '';
...
$('btnProtectGo').addEventListener('click', async () => {
  ...
  const w2 = $('protectWord2').value;
  const st = $('protectStatus');
  if (!w){ st.textContent = t('prot.needWord','Scrivi una parola.'); return; }
  if (w.length < 8){ st.textContent = t('prot.short','Troppo corta.'); return; }
  if (w !== w2){ st.textContent = t('prot.mismatch','Le due parole non sono uguali.'); return; }
...
/* ============================== how the app is doing ==============================
   ...
   guessing. */
const APP_VERSION = 'logos-modifica-3.78';
