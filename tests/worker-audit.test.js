...
/* =========================================================================
   v48 — la spia «Su Tor»: il relay dice alla pagina se la richiesta arriva da
   un'uscita di Tor (Cloudflare la marca con il paese «T1»). Un dato sulla
   richiesta, restituito solo a chi l'ha fatta; niente scritto, niente contato.
   ========================================================================= */
test.describe('worker: la spia «Su Tor»', () => {
  const ORIGINE = 'https://digitalvalut.github.io';
  const finta = () => Promise.resolve(new Response(JSON.stringify({ iceServers: [{ urls: 'turn:x' }] }), { status: 200 }));

  test('da un\'uscita Tor la risposta di /turn porta X-Logos-Tor: 1, da altrove 0', async () => {
    const w = W.caricaWorker({ fetch: finta });
    const tor = await w.chiama('GET', '/turn', { origin: ORIGINE, cf: { country: 'T1' } });
    assert.strictEqual(tor.status, 200);
    assert.strictEqual(tor.headers.get('X-Logos-Tor'), '1', 'chi arriva da Tor deve saperlo dal relay');
    const casa = await w.chiama('GET', '/turn', { origin: ORIGINE, cf: { country: 'IT' }, ip: '203.0.113.8' });
    assert.strictEqual(casa.headers.get('X-Logos-Tor'), '0', 'chi non e\' su Tor deve sapere anche quello');
  });

  test('senza request.cf (in locale, o un relay che non lo sa) l\'intestazione manca: «non lo so», non «no»', async () => {
    const w = W.caricaWorker({ fetch: finta });
    const r = await w.chiama('GET', '/turn', { origin: ORIGINE });
    assert.strictEqual(r.headers.get('X-Logos-Tor'), null);
  });

  test('anche dalla cache: la spia vale per QUESTA richiesta, non per quella di due minuti fa', async () => {
    const w = W.caricaWorker({ fetch: finta });
    await w.chiama('GET', '/turn', { origin: ORIGINE, cf: { country: 'IT' } });          /* riempie la cache */
    const tor = await w.chiama('GET', '/turn', { origin: ORIGINE, cf: { country: 'T1' }, ip: '203.0.113.9' });
    assert.strictEqual(tor.headers.get('X-Logos-Tor'), '1', 'il corpo e\' in cache, l\'intestazione no');
  });

  test('CORS: la pagina deve poter LEGGERE l\'intestazione, o la spia resta sempre grigia', async () => {
    const w = W.caricaWorker({ fetch: finta });
    const r = await w.chiama('GET', '/turn', { origin: ORIGINE, cf: { country: 'T1' } });
    assert.match(String(r.headers.get('Access-Control-Expose-Headers')), /X-Logos-Tor/);
  });
});
