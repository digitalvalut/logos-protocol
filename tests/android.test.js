...
/* =========================================================================
   v48 — il campanello e Tor (15 set 2026)
   ========================================================================= */
test.describe('v48: il campanello suona una volta, e mai mentre sei al telefono', () => {
  const RING = read(J + 'RingService.java');

  test('regola 1: se il telefono e\' in chiamata, il campanello non guarda nemmeno la cassetta', () => {
    assert.ok(/if \(!ringing && !CallService\.isInCall\(\) && !quietNow\(this\)\)/.test(RING),
      'prima di squillare: non sto gia\' squillando, non sono in chiamata, non ho appena gestito una chiamata');
    assert.ok(/static boolean isInCall\(\) \{ return inCall; \}/.test(CALL), 'CallService deve dire se siamo in chiamata');
  });

  test('regola 2: dopo una chiamata gestita, 150 secondi di silenzio per la stessa cassetta', () => {
    assert.ok(/QUIET_AFTER_HANDLED_MS = 150_000/.test(RING), '150 s: piu\' della vita di una busta (120 s), cosi\' la stessa non risuona');
    assert.ok(/putLong\(PREF_QUIET_UNTIL, System\.currentTimeMillis\(\) \+ QUIET_AFTER_HANDLED_MS\)/.test(RING), 'nei prefs, perche\' il servizio puo\' essere ricreato');
    const handled = RING.slice(RING.indexOf('if (ACTION_HANDLED.equals(action))'), RING.indexOf('return START_STICKY', RING.indexOf('if (ACTION_HANDLED.equals(action))')));
    assert.ok(/markHandled\(this\)/.test(handled), 'rispondere o rifiutare dallo schermo bloccato apre la finestra di silenzio');
  });

  test('la pagina avvisa il campanello quando prende in mano una chiamata (annuncio e risposta)', () => {
    assert.ok(/public void handled\(\)/.test(MAIN) && /RingService\.markHandled\(MainActivity\.this\)/.test(MAIN), 'il ponte AndroidRing.handled()');
    assert.ok(/function noteAndroidHandled\(\)/.test(APP) && /androidRing\.handled\(\)/.test(APP));
    const annuncio = APP.slice(APP.indexOf('addrPending = { msg, sec, slot: n };'), APP.indexOf('addrPending = { msg, sec, slot: n };') + 200);
    assert.ok(/noteAndroidHandled\(\)/.test(annuncio), 'quando l\'app annuncia la chiamata');
    const accetta = APP.slice(APP.indexOf('async function acceptAddrCall'), APP.indexOf('async function acceptAddrCall') + 900);
    assert.ok(/noteAndroidHandled\(\)/.test(accetta), 'e quando la accetta');
  });
});

test.describe('v48: Tor senza portarlo dentro', () => {
  test('il ponte AndroidTor: c\'e\' Orbot? accendilo (o apri la pagina ufficiale)', () => {
    for (const m of ['available', 'status', 'open']){
      assert.ok(new RegExp('@JavascriptInterface\\s+public [a-zA-Z]+ ' + m + '\\(').test(MAIN), 'AndroidTor.' + m + ' manca in MainActivity');
    }
    assert.ok(/addJavascriptInterface\(new TorBridge\(\), "AndroidTor"\)/.test(MAIN));
    assert.ok(/ORBOT_PKG = "org\.torproject\.android"/.test(MAIN) && /org\.torproject\.android\.intent\.action\.START/.test(MAIN));
    assert.ok(/https:\/\/www\.torproject\.org\/download\/#android/.test(MAIN), 'la pagina per installarlo e\' quella di Tor Project, non un negozio');
    assert.ok(/<queries>[\s\S]*org\.torproject\.android[\s\S]*<\/queries>/.test(MANIFEST), 'da Android 11 senza <queries> non si vede se Orbot c\'e\'');
    for (const m of ['status', 'open']) assert.ok(new RegExp('androidTor\\.' + m + '\\(').test(APP), 'la pagina usa AndroidTor.' + m);
  });

  test('niente Tor dentro l\'app: nessun binario, nessuna libreria, solo un\'app esterna accesa', () => {
    assert.ok(!/tor\.jar|tor-android|libtor|TorService|guardianproject/i.test(MAIN + CALL + MANIFEST), 'Logos non porta Tor dentro: lo fa Orbot');
  });
});
